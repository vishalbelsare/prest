...
class Workspace:
    ...
    def save_to_file(self, worker : Worker, fname: str) -> None:
        with bz2.open(fname, 'wb') as f_bz2:
            f = cast(FileOut, f_bz2)  # assert we're doing output
    ...
    def load_from_file(self, worker : Worker, fname: str) -> None:
        with bz2.open(fname, 'rb') as f_bz2:
            f = cast(FileIn, f_bz2)  # assert we're doing input
